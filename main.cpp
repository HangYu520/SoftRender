...
int main(int argc, char** argv)
{
    ...
    Engine::getInstance()->centerModel(model); // ! 影响 z 缓存
    while (window.isOpen())
    {
        ...
        while (const auto event = window.pollEvent())
        {
            ...
            if (const auto* keyPressed = event->getIf<sf::Event::KeyPressed>())
            {
                ...
                if (keyPressed->scancode == sf::Keyboard::Scan::R)
                {
                    ...
                    fov = 45.0f; // 重置视角范围
                    spdlog::info("Reset rotation, camera height, and field of view.");
                }
            }
        ...
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Up)) // 方向键上
            cameraHeight += 0.1f * deltaTime; // 交互式升降相机
        ...
        // !! 调用渲染引擎 !!
        {
            ...
            Engine::getInstance()->render(image, model); // 渲染
        }
